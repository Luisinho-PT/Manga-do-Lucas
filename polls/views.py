...
def fetch_recent_commits(after_commit=None, limit=5):
    """
    Pega os commits mais recentes da API do GitHub.
    Se after_commit for informado, traz commits a partir dele.
    """
    token = os.environ.get("GITHUB_TOKEN")
    ...
    for commit in data:
        ...
        iso_datetime = commit["commit"]["author"]["date"]  # ex: 2025-07-11T16:42:10Z
...
def main_page(request):
    ...
    if full_commit_hash:
        ...
        # Pega commits mais recentes a partir do commit atual (não incluindo ele)
        changelog = fetch_recent_commits(after_commit=short_hash, limit=5)
    else:
